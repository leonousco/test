...
(function () {
    ...
    Nis.View.UserInfoManagement = Uic.LayoutView.extend({
        className:'col-sm-12',
        id:'page_contents',
        regions: {
            header: '#page-title',
        ...
        showTitle:function() {
            var view = new Uic.View({raw:'<h3>회원 정보</h3>'});
        ...
        onConceptSetButtons:function(id, text, button) {
            ...
            switch(id) {
                case 'add': {
                    ...
                    this.trigger('click.addUser', id, text, button);
        ...
        onClickId:function(id, model, element){
            this.trigger('click.checkUserId', id, model, element);
        ...
        onSearchSuccess:function(result){
            ...
            var view = new Uic.FormLayout();
            var options = {
                collection:new Nis.Model.CCMModelList(result.body),
                columns:Nis.Model.CCMModel.userManagementColumns,   
                hasQuery:false,
                queryParam: {
                    column:'id',
            ...
            this.showConceptSetTableForm(options);
            // tableView.setCurrentPage(this.param.data.currentPage);      

            // table.collection.reset(result.body);
            // var collection = this.tableView.getCollection();
            // collection.reset(result.body);
        }
