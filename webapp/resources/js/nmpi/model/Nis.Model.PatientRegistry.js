...
(function () {
    ...
    Nis.Model.PatientRegistry = Nis.Model.extend({
        defaults: {
            id: null,
            dateCreated: "",
            dateVoided: "",
        ...
        initialize: function (options) {
            ...
            var newOptions = _.extend({
                entityName:Nis.Model.PatientRegistry.EntityName,
        ...
        sendMergePatientWithOid:function(options) {
            var url = this.getEntityUrl() + "/f_oid/" +options.from.oid + "/f_localId/" + options.from.localId
             + "/t_oid/" +options.to.oid + "/t_localId/" + options.to.localId;
            ...
            var options = {
                from: {
                    oid: options.prame.f_oid,
                    localId: options.prame.f_localId,
                },
                to: {
                    oid: options.prame.t_oid,
                    localId: options.prame.t_localId,
                },
                onSuccess:success,
                onError:error,
            }
    ...
    Nis.Model.PatientRegistry.EntityName = "PatientRegistry";

    Nis.Model.PatientRegistryList = Nis.Collection.extend({
        model: Nis.Model.PatientRegistry,
        initialize: function(){
